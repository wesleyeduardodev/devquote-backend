...
- **Anotações de faturamento (`billing_note`)** guardam texto rich-text + anexos próprios (`billing_note_attachments`, pasta S3 `billing-notes/{id}/attachments/`). `billing_period_id` **nulo = anotação geral**; preenchido = anotação daquele período. Endpoints `/api/billing-notes` (leitura ADMIN+MANAGER, escrita ADMIN) e `/api/billing-note-attachments`. ⚠️ Excluir período precisa limpar as notas antes (`BillingNoteService.deleteAllByBillingPeriod`, já chamado em `BillingPeriodServiceImpl.delete`/`deleteBulk`) — senão a FK barra o delete.
