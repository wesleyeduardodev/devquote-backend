...
import br.com.devquote.controller.doc.ServerControllerDoc;
...
public class ServerController implements ServerControllerDoc {

    private final ServerService serverService;

    private static final Set<String> ALLOWED_SORT_FIELDS = Set.of(
            "id", "name", "link", "createdAt", "updatedAt"
    );

    @Override
    @GetMapping
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<PagedResponse<ServerResponse>> list(
        ...
        Page<ServerResponse> pageResult = serverService.findAllPaginated(id, name, link, createdAt, updatedAt, pageable);
    ...
    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ServerResponse> create(@RequestBody @Valid ServerRequest dto) {
        return new ResponseEntity<>(serverService.create(dto), HttpStatus.CREATED);
    }

    @Override
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ServerResponse> update(@PathVariable Long id, @RequestBody @Valid ServerRequest dto) {
