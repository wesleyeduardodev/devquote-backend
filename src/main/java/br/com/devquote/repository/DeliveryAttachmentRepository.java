...
public interface DeliveryAttachmentRepository extends JpaRepository<DeliveryAttachment, Long> {
    ...
    @Query("SELECT COUNT(da) FROM DeliveryAttachment da WHERE da.delivery.id = :deliveryId")
    Long countByDeliveryId(@Param("deliveryId") Long deliveryId);

    @Query("SELECT da FROM DeliveryAttachment da WHERE da.filePath = :filePath")
    DeliveryAttachment findByFilePath(@Param("filePath") String filePath);
}